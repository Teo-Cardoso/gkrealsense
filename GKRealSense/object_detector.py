...
class ObjectDetector:
    def __init__(self, color_model_name: str, ir_model_name: str):
        ...
        self.color_classes = [ObjectType.BALL,
                              ObjectType.RED, ObjectType.BLUE, ObjectType.PERSON]

        self.ir_model = ultralytics.YOLO(ir_model_name, task="detect")
        self.ir_classes = [ObjectType.BALL,
                           ObjectType.PERSON, ObjectType.ROBOT]
    ...
    def _map_result_to_object(self, source_type: FramesMix, result: list[float]) -> DetectedObject:
        confidence = result[4]
        type = self._get_class(
            source_type == FramesMix.DEPTH_COLOR, int(result[5]))
        box = tuple(map(lambda x: int(x), result[:4]))
        return DetectedObject(type, confidence, box)

    def detect(self, source_type: FramesMix, source: np.ndarray) -> list[DetectedObject]:
        """Detect objects in the frame"""
        model: ultralytics.YOLO = self.color_model if source_type == FramesMix.DEPTH_COLOR else self.ir_model
        results: list[ultralytics.engine.results.Results] = model.predict(
            source=source, conf=0.5, verbose=False, device=0, half=True, int8=False, agnostic_nms=True, imgsz=640)
